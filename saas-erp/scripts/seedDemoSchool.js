...
async function seedDemoSchool() {
  console.log('🚀 Starting Demo School Complete Seeding...');
  ...
  // 3. Clear Existing Demo School Records to ensure clean slate
  ...
  for (const t of cleanTables) {
    try {
      ...
      console.warn(`Clean ${t}:`, e.message);
    }
  }

  // 4. Period Template & Rows
  ...
  const staffMembers = [
    ...
    { school_id: DEMO_SCHOOL_ID, full_name: 'Qari Abdul Rehman', role: 'teacher', designation: 'Islamiat & Arabic Head', qualification: 'M.A. Islamic Studies / Shahadat-ul-Almiya', email: 'rehman@edgexdemo.com', whatsapp_number: '03006667788', salary: 52000, is_active: true, created_at: SEED_TIMESTAMP },
    ...
    { school_id: DEMO_SCHOOL_ID, full_name: 'Mr. Kamran Siddiqui', role: 'accountant', designation: 'Finance & Accounts Officer', qualification: 'M.Com / MBA Finance', email: 'accounts@edgexdemo.com', whatsapp_number: '03008889900', salary: 70000, is_active: true, created_at: SEED_TIMESTAMP },
    { school_id: DEMO_SCHOOL_ID, full_name: 'Ms. Fatima Jameel', role: 'staff', designation: 'Front Desk & Gate Pass Officer', qualification: 'B.A. Public Administration', email: 'frontdesk@edgexdemo.com', whatsapp_number: '03009990011', salary: 45000, is_active: true, created_at: SEED_TIMESTAMP },
  ];
  ...
  const classMap = {};
  insertedClasses?.forEach(c => classMap[`${c.name}_${c.section}`] = c.id);
  ...
  for (const c of (insertedClasses || []).slice(0, 5)) {
  ...
  if (timetableRows.length > 0) {
    await supabase.from('timetable_slots').insert(timetableRows);
  }
  ...
  // 12. Fee Records for 12-Month Matrix
  console.log('💰 Seeding 12-Month Fee Ledger...');
  ...
  for (const st of insertedStudents || []) {
    ...
    feeMonthsList.forEach((m, mIdx) => {
      // Past months paid, current partial/paid, future unpaid
      const isPaid = mIdx < 7;
  ...
  // 15. Form Settings (Gate Pass & Lesson Planner)
  console.log('📄 Seeding Gate Pass & Lesson Planner Form Settings...');
  ...
  console.log('🎉 DEMO SCHOOL SEEDING COMPLETED SUCCESSFULLY!');
}
