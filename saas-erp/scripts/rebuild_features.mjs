...
content = content.replace(/return \{\s*student: stu, subjects: subjectRows, obtained, grand,\s*pct: overallG\.pct, grade: overallG\.grade, remarks: overallG\.remarks,\s*gpa: calculateGPA\(overallG\.grade\), fails, position: pos,\s*evaluation: evalMap\[stu\.id\] \|\| null,\s*attendance: presentDays > 0 \? `\$\{presentDays\} days` : 'N\/A',\s*\};/g, (match) => {
  if (!replaceReturn) {
    replaceReturn = true;
    return `return {
...
// 4. Update makeCardProps
content = content.replace(
  /const makeCardProps = \(card: any\) => \(\{\s*\.\.\.commonCardProps,\s*studentName: card\.student\.full_name,\s*rollNumber: String\(card\.student\.roll_number\),\s*studentPhoto: card\.student\.photograph_url \|\| null,\s*subjects: card\.subjects,\s*totalMarks: card\.grand,\s*obtainedMarks: card\.obtained,\s*percentage: card\.pct,\s*grade: card\.grade,\s*attendance: card\.attendance \|\| 'N\/A',\s*positionInClass: card\.position > 0 \? card\.position : undefined,\s*totalStudents: batchCards\.length,\s*finalStatus: card\.fails === 0 \? 'PROMOTED' : 'NOT PROMOTED',\s*evaluation: card\.evaluation \|\| undefined,\s*\}\);/,
...
// 5. Replace buttons
const oldButtons = `      {/* Action buttons */}
...
const newButtons = `      {/* General Action buttons (Exam level) */}
      {selectedExamType && (
...
// 6. Inject handlePrintMultipleClasses logic
...
if (!content.includes('const handlePrintMultipleClasses = async () => {')) {
  content = content.replace(/\/\/ Build batch cards for all students/, handlePrintMultipleClassesLogic + '  // Build batch cards for all students');
}
...
console.log('Successfully rebuilt all features perfectly.');
