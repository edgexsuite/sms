...
async function check() {
  ...
  for (const [cid, sList] of Object.entries(groups)) {
    const names = sList.map(s => s.subject_name.toUpperCase());
    if (names.includes('COMPUTER SCIENCE') && names.includes('ENGLISH') && names.includes('GENERAL SCIENCE')) {
      console.log(`Class Match: ${classMap[cid]} (${cid})`);
