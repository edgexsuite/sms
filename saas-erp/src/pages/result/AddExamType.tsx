...
export default function AddExamType() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    const [{ data: exams }, { data: pubData }] = await Promise.all([
      supabase
        .from('exam_types')
        .select('*')
    ...
    if (pubData?.sections_config && Array.isArray(pubData.sections_config.published_exam_ids)) {
      ...
      // Default initial state: existing exams are marked published so portals stay active
      setPublishedExamIds(examList.map((e: any) => e.id));
    }
  ...
  const handleTogglePublish = async (examId: string) => {
    ...
    try {
      const isCurrentlyPub = publishedExamIds.includes(examId);
      const nextIds = isCurrentlyPub
        ? publishedExamIds.filter(id => id !== examId)
        : [...publishedExamIds, examId];
  ...
  const handleBulkPublish = async (publishAll: boolean) => {
    if (!userRole?.school_id) return;
    const nextIds = publishAll ? examTypes.map(e => e.id) : [];
    setPublishedExamIds(nextIds);
    const { error } = await supabase.from('form_settings').upsert({
      school_id: userRole.school_id,
      form_name: 'exam_publication_settings',
    ...
    if (error) {
      alert('Failed to update bulk status: ' + error.message);
      fetchAll();
    }
  };
  ...
  const openEdit = (et: any) => {
    setEditId(et.id);
    setPublishImmediately(publishedExamIds.includes(et.id));
    setFormData({
      name:          et.name,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        ...
        // Also update publication status if changed
        const isPub = publishedExamIds.includes(editId);
        if (publishImmediately !== isPub) {
          const nextIds = publishImmediately
            ? [...publishedExamIds, editId]
            : publishedExamIds.filter(id => id !== editId);
          setPublishedExamIds(nextIds);
        ...
        const { data: inserted, error } = await supabase.from('exam_types').insert([payload]).select().single();
        if (error) throw error;
        if (publishImmediately && inserted) {
          const nextIds = [...publishedExamIds, inserted.id];
          setPublishedExamIds(nextIds);
          await supabase.from('form_settings').upsert({
            school_id: userRole?.school_id,
            form_name: 'exam_publication_settings',
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    if (publishedExamIds.includes(id)) {
      const nextIds = publishedExamIds.filter(i => i !== id);
      setPublishedExamIds(nextIds);
      await supabase.from('form_settings').upsert({
        school_id: userRole?.school_id,
        form_name: 'exam_publication_settings',
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm font-bold text-gray-800">All Exam Instances</span>
            ...
            <span className="text-xs bg-emerald-100 text-emerald-800 px-2.5 py-0.5 rounded-full font-bold flex items-center gap-1">
              ...
              {examTypes.filter(e => publishedExamIds.includes(e.id)).length} Published
            </span>
            <span className="text-xs bg-amber-100 text-amber-800 px-2.5 py-0.5 rounded-full font-bold flex items-center gap-1">
              <EyeOff className="w-3 h-3 text-amber-600" />
              {examTypes.filter(e => !publishedExamIds.includes(e.id)).length} Draft
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-3 py-1 rounded-lg transition flex items-center gap-1 cursor-pointer"
              title="Make all exams visible to parents & students"
            >
              <Globe className="w-3.5 h-3.5" /> Publish All
            </button>
            <button
              onClick={() => handleBulkPublish(false)}
              className="text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-200 px-3 py-1 rounded-lg transition flex items-center gap-1 cursor-pointer"
              title="Hide all exams from portals"
            >
              <EyeOff className="w-3.5 h-3.5" /> Unpublish All
            </button>
          </div>
        </div>
                ...
                {items.map((et, idx) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      {/* Publication Status Toggle Button */}
                      {publishedExamIds.includes(et.id) ? (
                        <button
                          type="button"
                          onClick={() => handleTogglePublish(et.id)}
                          disabled={togglingId === et.id}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition shadow-xs group cursor-pointer"
                          title="Click to Unpublish (Hide from Student and Parent Portals)"
                        >
                          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                          <Globe className="w-3.5 h-3.5 text-emerald-600" />
                          <span>Published</span>
                          <span className="hidden group-hover:inline text-[10px] text-emerald-800 underline ml-0.5">Unpublish</span>
                        </button>
                      ) : (
                        <button
                          type="button"
                          onClick={() => handleTogglePublish(et.id)}
                          disabled={togglingId === et.id}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200 hover:bg-amber-100 transition shadow-xs group cursor-pointer"
                          title="Click to Publish (Show on Student and Parent Portals)"
                        >
                          <EyeOff className="w-3.5 h-3.5 text-amber-600" />
                          <span>Draft (Hidden)</span>
                          <span className="hidden group-hover:inline text-[10px] text-amber-900 underline ml-0.5">Publish</span>
                        </button>
                      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-4 bg-gray-50">
              ...
              <div
                className="flex items-center justify-between p-3.5 rounded-xl border border-indigo-200 bg-indigo-50/60 cursor-pointer transition hover:bg-indigo-50"
              ...
              >
                <div className="pr-3">
                  ...
                  <p className="text-xs text-indigo-700/80 mt-0.5">
                    {publishImmediately
                      ? 'Results are visible to students and parents immediately.'
                      : 'Keep as Draft (hidden) while teachers enter and review marks.'}
                  </p>
                </div>
                {publishImmediately
                  ? <ToggleRight className="w-8 h-8 text-emerald-600 shrink-0" />
