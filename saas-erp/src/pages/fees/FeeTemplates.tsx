...
import { Layout, Plus, Trash2, Save, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function FeeTemplates() {
  ...
  useEffect(() => {
    if (userRole?.school_id) { fetchStructures(); fetchClasses(); }
  }, [userRole]);
  ...
  const fetchStructures = async () => {
    ...
    setStructures((data || []).map((s: any) => ({
      ...
      fee_matrix: s.fee_matrix || { recurrent: [{ item: 'Tuition Fee', amount: 0 }], first_time: [{ item: 'Admission Fee', amount: 0 }] },
    })));
    setLoading(false);
  };
  ...
  const addItem = (structureId: string, section: 'recurrent' | 'first_time') => {
    setStructures(prev => prev.map(s => s.id === structureId ? {
      ...s,
      fee_matrix: { ...s.fee_matrix, [section]: [...s.fee_matrix[section], { item: '', amount: 0 }] }
    } : s));
  };

  const removeItem = (structureId: string, section: 'recurrent' | 'first_time', idx: number) => {
    setStructures(prev => prev.map(s => s.id === structureId ? {
      ...s,
      fee_matrix: { ...s.fee_matrix, [section]: s.fee_matrix[section].filter((_, i) => i !== idx) }
    } : s));
  };

  const updateItem = (structureId: string, section: 'recurrent' | 'first_time', idx: number, field: 'item' | 'amount', value: string) => {
    setStructures(prev => prev.map(s => s.id === structureId ? {
      ...s,
      fee_matrix: {
        ...s.fee_matrix,
        [section]: s.fee_matrix[section].map((item, i) => i === idx ? { ...item, [field]: field === 'amount' ? parseFloat(value) || 0 : value } : item)
      }
    } : s));
  };
  ...
  const handleSave = async (structure: FeeStructure) => {
    ...
    await supabase.from('fee_structures').update({ fee_matrix: structure.fee_matrix, amount: totalRecurrent }).eq('id', structure.id);
    setSaving(null);
    fetchStructures();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        ...
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">
                  ...
                  <button onClick={() => setExpanded(isOpen ? null : s.id)}
                    ...
                    <div className="flex items-center gap-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Monthly: Rs. {recurrentTotal.toLocaleString()} · One-time: Rs. {firstTimeTotal.toLocaleString()}</p>
                      </div>
                    </div>
                      ...
                      {(['recurrent', 'first_time'] as const).map(section => (
                        <div key={section}>
                          <div className="flex justify-between items-center mb-3">
                            <h3 className="font-medium text-gray-700">{section === 'recurrent' ? 'Monthly / Recurring Fees' : 'One-Time Fees (Admission, etc.)'}</h3>
                            <button onClick={() => addItem(s.id, section)} className="text-indigo-600 text-sm flex items-center gap-1 hover:text-indigo-800">
                          ...
                          <div className="space-y-2">
                            {s.fee_matrix[section].map((item, idx) => (
                              <div key={idx} className="flex gap-3 items-center">
                                <input value={item.item} onChange={e => updateItem(s.id, section, idx, 'item', e.target.value)}
                                ...
                                <div className="relative">
                                  ...
                                  <input type="number" min="0" value={item.amount} onChange={e => updateItem(s.id, section, idx, 'amount', e.target.value)}
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
            ...
            <form onSubmit={handleAddClass} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Create Template</button>
              </div>
            </form>
          </div>
        </div>
